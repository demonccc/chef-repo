...
class Chef
  class Knife
    class Ssh_cheto < Knife
      ...
      banner "knife ssh_proxy QUERY COMMAND (options)"

      option :proxy_connect_command,
        :short => "-y COMMAND",
        :long => "--proxy-connect-command COMMAND",
        :description => "The command proxy to use for opening the connection",
        :default => nil
      ...
      def cssh
        hosts = ""
        session.servers_for.each do |server|
          hosts = hosts + "#{server.host} "
        end
        cssh_name = "'knife ssh #{@name_args[0]}'"
        exec("cssh --title #{cssh_name} -l #{config[:ssh_user]} #{hosts}")
      end
